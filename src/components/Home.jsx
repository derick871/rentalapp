...
function House() {
  ...
  return (
    <div className="bg-slate-50 min-h-screen flex m-10 p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Form Section */}
        <form onSubmit={handleSave} className="bg-white p-6 rounded-2xl shadow-xl shadow-slate-200/50 mb-16 grid grid-cols-1 md:grid-cols-5 gap-4 items-end border border-slate-100">
          <div className="md:col-span-1 flex flex-col">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Property Name</label>
            <input className="bg-slate-50 border-none ring-1 ring-slate-200 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. Skyline Heights" />
          </div>
          <div className="flex flex-col">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Location</label>
            <input className="bg-slate-50 border-none ring-1 ring-slate-200 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} placeholder="City/Town" />
          </div>
          <div className="flex flex-col">
            <label className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Standard</label>
            <input className="bg-slate-50 border-none ring-1 ring-slate-200 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={formData.standard} onChange={(e) => setFormData({...formData, standard: e.target.value})} placeholder="e.g. 3 BR" />
          </div>
          <div className="flex flex-col">
            ...
            <div key={house.id} className="group bg-white rounded-3xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 border border-slate-100 flex flex-col">
              ...
              <div className="p-6 flex-grow">
                ...
                <div className="flex items-center text-slate-400 text-sm gap-4 mt-4">
                  <span className="flex items-center gap-1">🛏️ {house.standard}</span>
                  <span className="flex items-center gap-1">📍 Near Center</span>
                </div>
